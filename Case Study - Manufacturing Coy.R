...
#top 2 regions which gave highest margin
names(sales3)
sales3 %>% group_by(region) %>% summarise(MARGIN = sum(margin)) %>% arrange(desc(MARGIN)) %>% head(2)

#top 2 partnumbers regions wise which gave highest margin
...
#BAR PLOT of Region wise sales
sales3 %>% group_by(region) %>% summarise(MARGIN = sum(margin)) %>% arrange(desc(MARGIN))
